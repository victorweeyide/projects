...
# create game objects
player = config.Character("player.png", 1, 100, 140)
player2 = config.Character("player2.png", 2, 130, 100)
score = 0
timer = 30
timer_counter = 0
    ...
    if timer > 0:
        ...
        for i in range(len(cheese_list)):
            if pygame.sprite.collide_rect(player, cheese_list[i]):
                ...
                print(f"After P1: {collided_list}")
            if pygame.sprite.collide_rect(player2, cheese_list[i]):
                ...
                collided_list.append(i)
                print(f"After P2: {collided_list}")
                    
            print(collided_list)
        for k in range(-1, -len(collided_list)-1, -1):
            if len(cheese_list) > 0:
                cheese_list.pop(collided_list[k])
